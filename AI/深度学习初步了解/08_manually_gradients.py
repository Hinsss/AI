...
y = tf.constant(housing.target.reshape(-1, 1), dtype=tf.float32, name='y') #真实值

# random_uniform函数创建图里一个节点包含随机数值，给定它的形状和取值范围，就像numpy里面rand()函数
theta = tf.Variable(tf.random_uniform([n + 1, 1], -1.0, 1.0), name='theta')
y_pred = tf.matmul(X, theta, name="predictions") #相乘得到预测的值
error = y_pred - y
mse = tf.reduce_mean(tf.square(error), name="mse") #对这列加和求平均  损失函数
# 梯度的公式：(y_pred - y) * xj
